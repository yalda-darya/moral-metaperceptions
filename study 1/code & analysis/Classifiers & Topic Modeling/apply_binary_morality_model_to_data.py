...
ID_TO_LABEL = {0: "non-moral", 1: "moral"}
...
if __name__ == "__main__":
    ...
    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=model_num_labels).to(
        device
    )
    ...
    # dtype of full_text, id_str, created_at, and prolife_prochoice are string
    ...
    abortion_dataset_complete_df.loc[:, "binary_morality"] = all_predictions
    abortion_dataset_complete_df.loc[:, "binary_morality"] = abortion_dataset_complete_df[
        "binary_morality"
    ].map(ID_TO_LABEL)
